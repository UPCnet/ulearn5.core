...
class ElasticSharing(object):
    ...
    def get(self, object, principal=None):
        ...
        # path = self.relative_path(object)
        self.elastic = getUtility(IElasticSearch)
        if self.elastic.connection is not None:
            ...
            if principal is None:
                ...
                es_results = self.elastic().search(
                    index=elastic_index,
    ...
    def modified(self, object):
        ...
        try:
            ...
            principals_to_delete = (
                set(existing_principals)
                - set(current_principals_in_groups)
                - set(current_principals)
            )
    ...
    def remove(self, object, principal):
        ...
        self.elastic().delete(index=elastic_index, id=result["hits"]["hits"][0]["_id"])
    ...
    def shared_with(self, username):
        ...
        communities_by_path = {
            a.getPath(): a
            for a in portal_catalog.unrestrictedSearchResults(
        ...
        def format_item(item):
            ...
            community_path = str(
                format(self.site_root_path) + "/" + item["path"].split("/")[1]
            )
            try:
                community = communities_by_path[community_path]
            except Exception as e:
                print("Error on shared_with: " + e)
                community = None

            item_catalog = portal_catalog.unrestrictedSearchResults(
                gwuuid=str(item["uuid"])
            )[0]
            return dict(
                ...
                by_profile="{}/profile/{}".format(
                    getSite().absolute_url(), item_catalog.Creator
                ),
            )
        ...
        def is_shared(item):
            ...
            if [
                group
                for group in groups
                if group in list(object.__ac_local_roles__.keys())
            ]:
                owner = object.getOwner()._id
                if username != owner:
                    return True
                ...
                is_Owner = [
                    a for a in object.__ac_local_roles__[username] if a in ["Owner"]
                ]
                if is_Owner:
                    return False
                effective_roles = [
                    a for a in object.__ac_local_roles__[username] if a not in ["Owner"]
                ]
                if effective_roles:
                    return True
            return False
        ...
        self.elastic = getUtility(IElasticSearch)
        elastic_index = ElasticSharing().get_index_name().lower()
        shared_items = self.elastic().search(
            index=elastic_index,
            body={
                "query": {
                    "bool": {
                        "must": {"match_all": {}},
                        "filter": {"terms": {"principal": principals}},
                    }
                },
                "size": 1000,
            },
...
class SharedWithMe(BrowserView):
