...
def searchUsersFunction(context, request, search_string):  # noqa
    ...
    if IPloneSiteRoot.providedBy(context):
        ...
        if search_string:
            ...
            ].subscriptions.get(qs={"username": search_string, "limit": 0})
            ...
            if max_users:
                merged_results = list(
                    set([plone_user.attrs["username"] for plone_user in plone_results])
                ...
                for plone_user in plone_results:
                    max_results = maxclientrestricted.contexts[
                        ...
                        qs={"username": plone_user.attrs["username"], "limit": 0}
                    )
                    merged_results_user = list(
                        set([plone_user.attrs["username"]])
    ...
    for user in users:
        ...
        if user is not None and user.attrs["username"] != "admin":
            if current_user.id == "admin":
                ...
                can_view_properties = (
                    current_user == user.attrs["username"]
            ...
            if isinstance(user, Record):
                ...
                if user_info:
                    ...
                    if has_extended_properties:
                        for (
                            user_property
                        ) in extended_user_properties_utility.properties:
                            if "check_" not in user_property:
                                check = user_info.getProperty(
                                    "check_" + user_property, ""
                                )
                                if can_view_properties or check == "" or check:
                                    user_dict.update(
                                        {
                                            user_property: user.attrs.get(
                                                user_property, ""
                                            )
                                        }
                                    )
                ...
                user_dict.update(dict(foto=str(userImage)))
                user_dict.update(
                    dict(url=portal.absolute_url() + "/profile/" + user.get("id", ""))
                )
                users_profile.append(user_dict)
